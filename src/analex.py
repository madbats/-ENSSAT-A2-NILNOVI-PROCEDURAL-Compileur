...
class LexicalUnit(object):
    ...
    def __str__(self):
        unitValue = {'classname': self.__class__.__name__, 'lIdx': self.line_index,
                     'cIdx': self.col_index, 'length': self.length, 'value': self.value}
...
class LexicalAnalyser(object):
            ...
            if c == '/':  # begin of comment or /= ...
                ...
                while colIndex < len(line) and (digit.match(c)):
                    ...
                    if colIndex < len(line):
                        c = line[colIndex]
                unitValue = Integer(lineIndex, beginColIndex,
                                    colIndex-beginColIndex, n)
            elif space.match(c):
                colIndex = colIndex + 1
            elif char.match(c):
                ...
                if string_is_keyword(ident):
                    unitValue = Keyword(
                        lineIndex, beginColIndex, len(ident), ident)
                else:
                    unitValue = Identifier(
                        lineIndex, beginColIndex, len(ident), ident)
            elif c == ':':  # affectation
                beginColIndex = colIndex
                colIndex = colIndex + 1
    ...
    def acceptKeyword(self, keyword):
        ...
        if self.lexical_units[self.lexical_unit_index].is_keyword(keyword):
            ...
            raise AnaLexException("Expecting keyword "+keyword+" <line "+str(self.lexical_units[self.lexical_unit_index].get_line_index(
            ))+", column "+str(self.lexical_units[self.lexical_unit_index].get_col_index())+"> !")
    ...
    def acceptIdentifier(self):
        if not self.verify_index():
            raise AnaLexException(
                "Found end of entry while identifer expected!")
        if self.lexical_units[self.lexical_unit_index].is_identifier():
            value = self.lexical_units[self.lexical_unit_index].get_value()
            self.lexical_unit_index += 1
            return value
        else:
            raise AnaLexException("Expecting identifier <line "+str(self.lexical_units[self.lexical_unit_index].get_line_index(
            ))+", column "+str(self.lexical_units[self.lexical_unit_index].get_col_index())+"> !")
    ...
    def acceptInteger(self):
        if not self.verify_index():
            raise AnaLexException(
                "Found end of entry while integer value expected!")
        if self.lexical_units[self.lexical_unit_index].is_integer():
            value = self.lexical_units[self.lexical_unit_index].get_value()
            self.lexical_unit_index += 1
            return value
        else:
            raise AnaLexException("Expecting integer <line "+str(self.lexical_units[self.lexical_unit_index].get_line_index(
            ))+", column "+str(self.lexical_units[self.lexical_unit_index].get_col_index())+"> !")
    ...
    def acceptFel(self):
        ...
        if self.lexical_units[self.lexical_unit_index].is_fel():
            ...
            raise AnaLexException("Expecting end of program <line "+str(self.lexical_units[self.lexical_unit_index].get_line_index(
            ))+", column "+str(self.lexical_units[self.lexical_unit_index].get_col_index())+"> !")
    ...
    def acceptCharacter(self, c):
        ...
        if self.lexical_units[self.lexical_unit_index].is_character(c):
            ...
            raise AnaLexException("Expecting character " + c + " <line "+str(self.lexical_units[self.lexical_unit_index].get_line_index(
            ))+", column "+str(self.lexical_units[self.lexical_unit_index].get_col_index())+"> !")
    ...
    def acceptSymbol(self, s):
        ...
        if self.lexical_units[self.lexical_unit_index].is_symbol(s):
            ...
            raise AnaLexException("Expecting symbol " + s + " <line "+str(self.lexical_units[self.lexical_unit_index].get_line_index(
            ))+", column "+str(self.lexical_units[self.lexical_unit_index].get_col_index())+"> !")
    ...
    def isInteger(self):
        if not self.verify_index():
            raise AnaLexException(
                "Found end of entry while expecting integer value!")
        if self.lexical_units[self.lexical_unit_index].is_integer():
            return True
        return False
